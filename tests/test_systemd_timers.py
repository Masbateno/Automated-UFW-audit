...
from ufw_audit.checks.systemd_timers import (
...
class TestCheckSystemdTimers:
    ...
    def test_long_exec_start_truncated_in_detail(self):
        ...
        from ufw_audit.checks.systemd_timers import _MAX_EXEC_LENGTH
...
"""
    ...
    def test_parses_timer_names(self):
        with patch("ufw_audit.checks.systemd_timers._run", return_value=self.SAMPLE_OUTPUT):
    ...
    def test_derives_service_when_missing(self):
        ...
        with patch("ufw_audit.checks.systemd_timers._run", return_value=out):
            units = _list_timer_units()
    ...
    def test_empty_output(self):
        with patch("ufw_audit.checks.systemd_timers._run", return_value=""):
...
class TestFindServiceFile:
    def test_finds_etc_first(self, tmp_path):
        ...
        import ufw_audit.checks.systemd_timers as mod
...
class TestPipeToShellDetection:
    ...
    def test_detection(self, cmd, should_match):
        from ufw_audit.checks.systemd_timers import _DOWNLOADER_RE, _PIPE_TO_SHELL_RE
...
class TestFromSystem:
    def test_no_systemctl_returns_empty(self):
        with patch("ufw_audit.checks.systemd_timers._command_exists", return_value=False):
    ...
    def test_detects_pipe_to_shell(self, tmp_path):
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run", return_value="  evil.timer  evil.service\n"),
            ):
                snap = SystemdTimersSnapshot.from_system()
        finally:
            mod._SERVICE_DIRS = orig_dirs
    ...
    def test_detects_world_writable_script(self, tmp_path):
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run", return_value="  timer.timer  timer.service\n"),
            ):
                snap = SystemdTimersSnapshot.from_system()
        finally:
            mod._SERVICE_DIRS = orig_dirs
    ...
    def test_user_created_root_timer(self, tmp_path):
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run", return_value="  custom.timer  custom.service\n"),
            ):
                snap = SystemdTimersSnapshot.from_system()
        finally:
            mod._SERVICE_DIRS = orig_dirs
    ...
    def test_timer_with_user_directive_not_flagged(self, tmp_path):
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run", return_value="  managed.timer  managed.service\n"),
            ):
                snap = SystemdTimersSnapshot.from_system()
        finally:
            mod._SERVICE_DIRS = orig_dirs
    ...
    def test_max_timers_cap_prevents_processing_beyond(self, tmp_path):
        ...
        from ufw_audit.checks.systemd_timers import _MAX_TIMERS
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run", return_value="".join(lines)),
            ):
                snap = SystemdTimersSnapshot.from_system()
        finally:
            mod._SERVICE_DIRS = orig_dirs
    ...
    def test_script_dedup_across_timers(self, tmp_path):
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run",
    ...
    def test_multi_exec_start_flags_if_any_dangerous(self, tmp_path):
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run", return_value="  mixed.timer  mixed.service\n"),
            ):
                snap = SystemdTimersSnapshot.from_system()
        finally:
            mod._SERVICE_DIRS = orig_dirs
    ...
    def test_user_root_explicit_not_in_root_timers(self, tmp_path):
        ...
        try:
            with (
                patch("ufw_audit.checks.systemd_timers._command_exists", return_value=True),
                patch("ufw_audit.checks.systemd_timers._run", return_value="  myapp.timer  myapp.service\n"),
            ):
                snap = SystemdTimersSnapshot.from_system()
        finally:
            mod._SERVICE_DIRS = orig_dirs
