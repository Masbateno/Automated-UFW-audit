...
from ufw_audit import output as output_mod
from ufw_audit.output import _passes_threshold, init as output_init
...
class TestDisplayResultFiltering:
    ...
    def _make_findings(self):
        from ufw_audit.scoring import Finding, FindingLevel
        return [
    ...
    def test_min_level_warn_suppresses_ok_from_terminal(self):
        ...
        with patch("ufw_audit.output.print_ok") as mock_ok:
    ...
    def test_min_level_warn_suppresses_info_from_terminal(self):
        ...
        from ufw_audit.display import display_result
        with patch("ufw_audit.output.print_info") as mock_info:
            display_result(result, report, verbose=False)
    ...
    def test_min_level_warn_still_shows_warn(self):
        ...
        from ufw_audit.display import display_result
        with patch("ufw_audit.output.print_warn") as mock_warn:
            display_result(result, report, verbose=False)
    ...
    def test_min_level_warn_still_shows_alert(self):
        ...
        from ufw_audit.display import display_result
        with patch("ufw_audit.output.print_alert") as mock_alert:
            display_result(result, report, verbose=False)
    ...
    def test_no_threshold_shows_all(self):
        ...
        from ufw_audit.display import display_result
        with (
            patch("ufw_audit.output.print_ok")    as m_ok,
            patch("ufw_audit.output.print_info")  as m_info,
            patch("ufw_audit.output.print_warn")  as m_warn,
            patch("ufw_audit.output.print_alert") as m_alert,
        ):
            display_result(result, report, verbose=False)
    ...
    def test_output_order_respected(self):
        ...
        from ufw_audit.display import display_result
        with (
            patch("ufw_audit.output.print_ok",    side_effect=lambda *a, **kw: calls.append("ok")),
            patch("ufw_audit.output.print_info",  side_effect=lambda *a, **kw: calls.append("info")),
            patch("ufw_audit.output.print_warn",  side_effect=lambda *a, **kw: calls.append("warn")),
            patch("ufw_audit.output.print_alert", side_effect=lambda *a, **kw: calls.append("alert")),
        ):
            display_result(result, report, verbose=False)
...
class TestScoreTrend:
    ...
    def _score_str_for(self, score: int, prev: "int | None") -> str:
        ...
        from ufw_audit import output as out_mod
        ...
        with patch("ufw_audit.output.print_summary_box", side_effect=fake_box):
            print_audit_summary(
