"""
Unit tests for ufw_audit.sysinfo — get_public_ip() and detect_network_context().
"""
...
from ufw_audit.sysinfo import detect_network_context, get_public_ip
...
class TestDetectNetworkContext:
    ...
    def test_private_gateway_returns_local(self):
        ...
        with patch("subprocess.run") as mock_run, \
             patch("ufw_audit.sysinfo.get_public_ip", return_value="1.2.3.4") as mock_gip:
    ...
    def test_private_gateway_offline_skips_lookup(self):
        ...
        with patch("subprocess.run", return_value=self._run_result(route_out)), \
             patch("ufw_audit.sysinfo.get_public_ip", return_value="") as mock_gip:
    ...
    def test_link_local_ipv6_not_treated_as_public(self):
        ...
        with patch("subprocess.run", side_effect=run_side), \
             patch("ufw_audit.sysinfo.get_public_ip", return_value=""):
            ctx, ip = detect_network_context()
