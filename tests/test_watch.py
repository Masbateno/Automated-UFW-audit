"""
Tests for ufw_audit/watch.py — --watch mode.
...
from ufw_audit.watch import _NullReport, _score_bar
...
class TestWatchKeyboardInterrupt:
    ...
    def test_keyboard_interrupt_during_sleep_returns_0(self):
        ...
        with (
            patch("ufw_audit.watch.detect_network_context", return_value=(MagicMock(), None)),
            patch("ufw_audit.watch.run_checks",             return_value=minimal_result),
            patch("ufw_audit.watch.ScoreEngine",            return_value=fake_engine),
            patch("ufw_audit.watch.build_baseline",         return_value=fake_baseline),
            patch("ufw_audit.watch.save_baseline"),
            patch("ufw_audit.watch.time.sleep",             side_effect=KeyboardInterrupt),
        ):
            rc = run_watch(
                config,
                interval=10,
    ...
    def test_keyboard_interrupt_calls_stopped_message(self):
        ...
        with (
            ...
            patch("ufw_audit.watch.run_checks",             return_value=minimal_result),
            patch("ufw_audit.watch.ScoreEngine",            return_value=fake_engine),
            patch("ufw_audit.watch.build_baseline",         return_value=MagicMock()),
            patch("ufw_audit.watch.save_baseline"),
            patch("ufw_audit.watch.time.sleep",             side_effect=KeyboardInterrupt),
        ):
            run_watch(
