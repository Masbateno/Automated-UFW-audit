"""
Unit tests for ufw_audit.cron module.
...
class TestParseCronFile:
    def _write_cron(self, tmp_path, content, name="ufw-audit-test"):
        p = tmp_path / name
    ...
    def test_parses_v013_cron(self, tmp_path):
        ...
        assert entry.script_path == Path("/usr/local/bin/ufw-audit-nightly")
    ...
    def test_parses_no_email(self, tmp_path):
        content = (
            ...
            "# email: \n"
            "SHELL=/bin/bash\n\n"
            "0 2 * * 1  root  /usr/local/bin/ufw-audit-weekly\n"
        )
        p = self._write_cron(tmp_path, content, "ufw-audit-weekly")
        entry = parse_cron_file(p)
        assert entry is not None
        assert entry.email == ""
    ...
    def test_legacy_cron_name_fallback(self, tmp_path):
        content = (
            "# UFW-AUDIT daily audit — generated 2025-01-01\n"
            "SHELL=/bin/bash\n\n"
            "0 3 * * *  root  /usr/local/bin/ufw-audit-nightly\n"
        )
        p = self._write_cron(tmp_path, content, "ufw-audit")
    ...
    def test_name_derived_from_filename_when_no_metadata(self, tmp_path):
        content = (
            "SHELL=/bin/bash\n\n"
            "0 3 * * *  root  /usr/local/bin/ufw-audit-monthly\n"
        )
        p = self._write_cron(tmp_path, content, "ufw-audit-monthly")
        entry = parse_cron_file(p)
        assert entry is not None
...
class TestParseDayNames:
    def test_single_day_en(self):
        from ufw_audit.cron import _DAYS_EN
