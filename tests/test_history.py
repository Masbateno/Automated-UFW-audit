...
class TestSaveLoadHistory:
    def test_save_creates_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("ufw_audit.history._CONFIG_DIR", tmp_path)
        monkeypatch.setattr("ufw_audit.history._HISTORY_FILE", tmp_path / "history.jsonl")
        save_score(8, "low")
    ...
    def test_save_writes_valid_json(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("ufw_audit.history._HISTORY_FILE", hf)
        save_score(9, "low")
    ...
    def test_load_returns_empty_if_no_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("ufw_audit.history._HISTORY_FILE", tmp_path / "missing.jsonl")
        assert load_history() == []
