"""Tests for ufw_audit/html_output.py — --html export."""
...
from ufw_audit.html_output import build_html_output, _h, _score_color
from ufw_audit.cli import CLIError, parse_args
...
def _make_sys_info(**kw):
    ns = SimpleNamespace(
        ...
        user="so6",
        config_path="/root/.config/ufw-audit/config.conf",
        language="en",
        version="1.21.0",
    )
    for k, v in kw.items():
...
class TestHtmlStructure:
    ...
    def test_footer_present(self):
        ...
        assert "ufw-audit" in footer.text
