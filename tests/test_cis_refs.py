"""
Tests for ufw_audit/cis_refs.py — CIS benchmark reference lookup.
...
from ufw_audit.cis_refs import _load, get_cis_code, get_cis_ref


_DATA_FILE = Path(__file__).parent.parent / "ufw_audit" / "data" / "cis_refs.json"
...
class TestLoadCache:
    ...
    def test_load_returns_empty_dict_on_missing_file(self, monkeypatch, tmp_path):
        import ufw_audit.cis_refs as mod
...
class TestNoStaleExplainCis:
    def test_en_json_has_no_explain_cis(self):
        en_path = Path(__file__).parent.parent / "ufw_audit" / "locales" / "en.json"
    ...
    def test_fr_json_has_no_explain_cis(self):
        fr_path = Path(__file__).parent.parent / "ufw_audit" / "locales" / "fr.json"
