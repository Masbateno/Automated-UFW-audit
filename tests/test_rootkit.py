...
from ufw_audit.checks.rootkit import (
    RootkitSnapshot,
    _rkhunter_db_age,
    _rkhunter_last_scan,
...
class TestRootkitFromSystemRkhunter:
    ...
    def test_tool_rkhunter_takes_priority_over_chkrootkit(self):
        ...
        with patch("ufw_audit.checks.rootkit._command_exists", side_effect=_cmd), \
             patch("ufw_audit.checks.rootkit._rkhunter_db_age", return_value=1), \
             patch("ufw_audit.checks.rootkit._rkhunter_last_scan", return_value="2026-04-11"):
            snap = RootkitSnapshot.from_system()
        assert snap.tool == "rkhunter"
