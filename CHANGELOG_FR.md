...
| Version | Date | Résumé |
|---------|------|--------|
| [v1.25.0](#v1250) | 2026-04-26 | Cartographie CIS inline (`[CIS:X.Y.Z]` par finding, ref complète en `--verbose`) ; préfixe best-practice ; `cis_refs.json` restructuré ; `get_cis_code()` ; `explain_cis` supprimé des locales ; 5 nouveaux services (SMTP, NFS, Jenkins, OpenVPN, Squid) ; correctif `_ipt_has_conntrack` ACCEPT ; FORWARD DROP/REJECT → OK ; 4200/4200 tests (+60) |
| [v1.24.1](#v1241) | 2026-04-25 | Hotfix : parsing noyau Debian (`_KVER_RE` `[-+]`, ABI optionnel, chemin apt `linux-image-$(uname -r)`) ; correction changelog v1.24.0 (INPUT ACCEPT → ALERTE −3 pts) ; 4140/4140 tests (+6) |
...
- **Cartographie CIS inline** — chaque finding disposant d'un code CIS formel affiche désormais `[CIS:X.Y.Z]` (estompé) dans la boîte de synthèse ; les entrées best-practice (sans numéro de section formel) sont laissées sans étiquette
- **Ref CIS complète en `--verbose`** — la chaîne de référence CIS complète est affichée estompée après chaque ligne de finding WARN/ALERT (ex. `CIS Ubuntu 22.04 L1 — 5.2.7 — Ensure SSH MaxAuthTries is set to 4 or less`)
- **Entrées best-practice renommées** — 34 entrées sans numéro de section CIS (contournement virtualisation, authentification par clé SSH, etc.) utilisent désormais le préfixe `"Best practice — ..."` au lieu du préfixe trompeur `"CIS ..."` ; `get_cis_ref()` retourne la chaîne appropriée selon la catégorie
- **`cis_refs.json` restructuré** (`data/cis_refs.json`) — chaque entrée est désormais `{"ref": "...", "code": "CIS:X.Y.Z"|null}` au lieu d'une chaîne plate ; 133 entrées au total : 99 CIS Ubuntu 22.04 (`code: "CIS:X.Y.Z"`), 4 CIS Docker (`code: "CIS Docker:X.Y"`), 34 best-practice (`code: null`)
- **`get_cis_code()`** (`cis_refs.py`) — nouvelle fonction retournant le code court machine-readable (ex. `"CIS:5.2.7"`, `"CIS Docker:5.4"`) ou `None` pour les entrées best-practice
- **`explain.py` refs CIS découplées des fichiers locale** — `run_explain()` et le TUI curses appellent désormais `get_cis_ref(key)` directement au lieu de `t("explain_cis.{key}")` ; la section `explain_cis` (170 chaînes) a été supprimée de `en.json` et `fr.json`
- **5 nouveaux services** (`data/services.json`) — SMTP/Postfix/Exim (25/tcp, high), NFS Server (2049/tcp+udp, high), Jenkins (8080/tcp, high), OpenVPN (1194/udp, medium), Squid Proxy (3128/tcp, medium) ; le registre couvre désormais **32 services**
- **Correctif `_ipt_has_conntrack` ACCEPT** (`checks/iptables_nftables.py`) — regex étendu pour exiger `-j ACCEPT` sur la même ligne ; auparavant `--ctstate ESTABLISHED,RELATED -j DROP` était incorrectement traité comme une règle conntrack valide
- **FORWARD DROP/REJECT → `result.ok()`** (`checks/iptables_nftables.py`) — quand la politique FORWARD est DROP ou REJECT, émet ✔ [OK] ; symétrique avec INPUT ; clé locale `forward_ok` ajoutée

### Tests

| Fichier | Modification |
|---------|-------------|
| `tests/test_cis_refs.py` | nouveau — 39 tests : `TestGetCisRef` (12), `TestGetCisCode` (11), `TestLoadCache` (4), `TestJsonSchema` (10), `TestNoStaleExplainCis` (2) |
| `tests/test_domain_scores.py` | `TestCISReferences` mis à jour — `test_explain_cis_all_keys_resolve` accepte les refs best-practice ; `test_explain_cis_locale_independent` renommé |
| `tests/test_iptables_nftables.py` | `TestIptableParsers` (+2 : action DROP conntrack fausse) ; `TestForwardPolicy` (+4 : forward ok DROP/REJECT, niveau OK, zéro déduction) |
| `tests/test_services.py` | `TestNewServicesRegistry` (+15 : smtp/nfs/jenkins/openvpn/squid — existence, risque, ports) |
...
- **Parsing noyau Debian** (`checks/kernel_modules.py`) — `_KVER_RE` étendu de `[-]` à `[-+]` pour parser les versions Debian (`6.12.74+deb13+1-amd64`) ; `_kernel_sort_key` réécrit pour gérer l'absence du groupe ABI numérique (Debian sépare avec `+` et non `-ABI`) ; `_query_apt_kernel_update` ajoute `apt-cache policy linux-image-$(uname -r)` comme second chemin principal pour Debian (pas de méta-paquet `linux-image-generic`) ; commande apt upgrade généralisée en `sudo apt upgrade`
- **Correction changelog v1.24.0** — INPUT ACCEPT → ALERTE −3 pts (pas ⚠ −1 pt) ; FORWARD ACCEPT reste ⚠ −1 pt ; `iptables -S` (pas `-L`) documenté ; "backend actif" → "couche firewall disponible"

### Tests

| Fichier | Modification |
|---------|-------------|
| `tests/test_kernel_modules.py` | `TestKernelSortKey` (+2), `TestParseInstalledKernels` (+2), `TestKernelAptUpdate` (+2) — tri version Debian, parsing séparateur `+`, apt à jour sur Debian |
...
- **CHECK 46 — audit iptables/nftables** (`checks/iptables_nftables.py`) — quand UFW est inactif, audite la couche pare-feu sous-jacente ; détecte la couche firewall disponible (nft list ruleset ou règles iptables -S) ; vérifie les politiques par défaut INPUT/FORWARD (iptables -S / nft list ruleset) ; contrôle la présence de la règle conntrack ESTABLISHED,RELATED ACCEPT ; INPUT ACCEPT → ALERTE −3 pts ; FORWARD ACCEPT → ⚠ −1 pt ; conntrack absent → ⚠ −1 pt ; section conditionnée sur `not fw_status.active`
...
- `CorrelationRule` : frozensets `all_of` / `any_of` de clés de findings ; se déclenche quand `all_of ⊆ actifs` ET (`any_of` vide OU `any_of ∩ actifs ≠ ∅`)
- `run_correlations(engine, t)` → liste de `CorrelatedFinding` (key, level, message, triggered_by)
- 5 règles intégrées :
  - `corr.root_no_protection` — SSH PermitRootLogin + pas de Fail2ban → ALERT
  - `corr.password_auth_under_attack` — auth par mot de passe activée + attaque brute-force détectée → ALERT
  - `corr.ssh_root_password` — connexion root + mot de passe → ALERT
  - `corr.privilege_escalation` — sudoers NOPASSWD + SUID inattendu → WARN
  - `corr.stale_unmonitored` — mises à jour sécurité en attente + pas de Fail2ban → WARN
  - `corr.fully_blind` — journalisation UFW désactivée + pas de Fail2ban + pas d'auditd → WARN
- 49 tests dans `tests/test_correlation.py`

### Suivi des findings récurrents (`recurrence.py`)

- `load_recurrence()` / `save_recurrence()` — compteurs JSONL dans `~/.config/ufw-audit/recurrence.json`
- `update_recurrence(prev, active_keys)` — incrémente par clé ; les clés absentes sont supprimées (finding résolu)
- Valeurs corrompues / négatives normalisées à 0 avant l'incrément ; clés vides et valeurs négatives filtrées au chargement
- Écriture atomique avec nettoyage `tmp.unlink(missing_ok=True)` en cas d'échec
- 27 tests dans `tests/test_recurrence.py`

### Analyse d'exposition des ports (`exposure.py`)

- Regroupe les services en écoute exposés par portée d'interface et niveau de risque
- Correctif politique `fw_policy` : utilise `not in ("deny", "reject")` — politique inconnue/None traitée comme permissive
- Attribut direct `lp.port` pour le filtre ports éphémères (reparsing de la chaîne `port_proto` supprimé)
- 43 tests dans `tests/test_exposure.py`

### Rapport comparatif — diff de clés de findings (`compare.py`)

- `AuditBaseline.finding_keys` — nouveau champ : liste triée des clés ALERT/WARN actives sauvegardées avec chaque baseline
- `AuditDelta.new_finding_keys` / `resolved_finding_keys` — clés apparues ou résolues depuis le dernier audit
- Garde de migration : si `prev.finding_keys` est vide (baseline antérieure à v1.22), le diff est ignoré — évite le flood de faux "nouveaux findings" à la première exécution après mise à jour
- `display_delta()` affiche chaque nouvelle clé (WARN) et chaque clé résolue (OK)

### Correctifs

- **Couleur exposition des ports (`exposure.py`)** : ports colorés `alert` quand `fw_policy` n'est pas `deny`/`reject` (était `"allow"` uniquement — `"unknown"` et `None` produisaient incorrectement `warn`)

- **Faux positif IPv6 (`checks/ipv6.py`)** : champ `has_global_ipv6` + parseur `_read_global_ipv6()` ; quand UFW IPv6 est désactivé et que des listeners existent mais uniquement des adresses link-local/ULA, le WARN −2 pts est rétrogradé en INFO — la machine n'est pas joignable via IPv6 depuis internet
- **Message noyaux obsolètes (`checks/kernel_modules.py`)** : quand le noyau actif est égal au plus récent installé, la parenthèse redondante "(actif : X, récent : X)" est supprimée — utilise la nouvelle clé locale `kernels_obsolete_same`
- **Certificat snakeoil (`checks/ssl_certs.py`)** : `ssl-cert-snakeoil.pem` de Debian/Ubuntu filtré lors du scan de `/etc/ssl/private` — le certificat de test système ne déclenche plus l'audit TLS
- **Avertissement services implicites (`display.py`)** : message `implicit_svcs` supprimé quand `fw_policy` est `deny` ou `reject` — les politiques default-deny bloquent déjà les services non déclarés
- **Note port SSH non-standard (`runner.py`)** : note "Port non standard — exposition réduite aux scanners automatisés" affichée dans la section services quand SSH tourne sur un port autre que 22

### Passage qualité

- **`domain_scores.py`** : domaine `backup` déplacé de `hardening` vers `disk` — les solutions de sauvegarde appartiennent au scoring santé disque
- **`explain.py`** : 87→112 clés (+25 réparties sur 7 nouveaux groupes : Journaux d'authentification, Umask, Journalisation du pare-feu, Certificats TLS/SSL, Timers Systemd, Firmware, Docker)
- **`__main__.py`** : `fw_policy` transmis à `print_audit_summary`

### Tests

- `tests/test_correlation.py` — 49 tests (nouveau)
- `tests/test_exposure.py` — 50 tests (nouveau)
- `tests/test_recurrence.py` — 27 tests (nouveau)
- `tests/test_ipv6.py` — +26 tests (`TestReadGlobalIPv6` : unicast global, link-local, ULA, loopback, monkey-patch `_run`)
- `tests/test_explain.py` — assertion du nombre de clés mise à jour (87→112)
