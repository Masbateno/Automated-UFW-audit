...
- **Parsing version noyau Debian** (`ufw_audit/checks/kernel_modules.py`) — regex `_KVER_RE` étendu de `^(\d+)\.(\d+)\.(\d+)-(\d+)` à `^(\d+)\.(\d+)\.(\d+)[-+]` pour gérer les versions noyau Debian où le séparateur est `+` et non `-` (ex. `6.12.74+deb13+1-amd64`) ; `_kernel_sort_key` réécrit — après le `[-+]`, la chaîne restante est scannée pour un entier de tête optionnel (numéro ABI) ; les versions Debian n'ayant pas d'entier après `+`, `abi` vaut `0` et le tri reste correct sur Ubuntu et Debian ; `_query_apt_kernel_update` ajoute un second chemin principal : `apt-cache policy linux-image-$(uname -r)` pour les systèmes sans méta-paquet `linux-image-generic` (Debian utilise le nom de paquet versionné complet) ; commande apt upgrade généralisée de `sudo apt upgrade linux-image-generic` à `sudo apt upgrade` ; effet combiné sur Debian 13 : sur les systèmes où les trois problèmes sont présents, `installed_kernels` devient vide → early return → finding apt jamais atteint → pas de ✔ [OK] même noyau à jour
- **Correction changelog v1.24.0** — CHANGELOG.md, CHANGELOG_FR.md, CHANGELOG_FULL.md, CHANGELOG_FULL_FR.md corrigés : INPUT ACCEPT émet `result.alert(points=3)` — ALERTE −3 pts, pas ⚠ −1 pt ; FORWARD ACCEPT reste ⚠ −1 pt ; `iptables -S` (pas `iptables -L INPUT --line-numbers`) documenté ; "détecte le backend actif" révisé en "détecte la couche firewall disponible"

### Tests

| Fichier | Classe | Modification |
|---------|--------|-------------|
| `tests/test_kernel_modules.py` | `TestKernelSortKey` | +2 tests — `test_debian_style_version_parseable` (`6.12.74+deb13+1-amd64` → `(6, 12, 74, 0)`), `test_debian_sorts_correctly_against_ubuntu` (Debian > Ubuntu plus ancien) |
...
✅ 4140/4140 tests unitaires (+6 depuis v1.24.0)
...
- **CHECK 46 — audit iptables/nftables** (`ufw_audit/checks/iptables_nftables.py`) — nouveau check conditionné sur `not fw_status.active` ; quand UFW est inactif, audite la couche pare-feu sous-jacente : infère la couche firewall active (nftables en priorité, repli sur iptables) ; vérifie la politique par défaut INPUT (`iptables -S` / `nft list ruleset`) ; vérifie la politique FORWARD ; contrôle la présence d'un pattern de règle conntrack ESTABLISHED/RELATED ; ⛔ ALERTE −3 pts pour INPUT ACCEPT (exposition critique) ; ⚠ −1 pt pour FORWARD ACCEPT ; ⚠ −1 pt pour conntrack absent ; FORWARD `unknown` → INFO (pas de déduction) ; nouvelle section `IPTABLES / NFTABLES AUDIT` dans runner ; nouvelle section locale `iptables_nft.*` dans en.json / fr.json
- **Profil d'audit dans le bandeau** — le profil actif (`server` / `desktop` / `container`) est affiché en ℹ [INFO] immédiatement après le bandeau, avant toute section
- **5 nouveaux services critiques** (`ufw_audit/data/services.json`) — Telnet Server (23/tcp, `inetutils-telnetd` / `telnetd`, binaires `in.telnetd` / `telnetd`) ; RDP/xRDP (3389/tcp, `xrdp`) ; MongoDB (27017/tcp, `mongodb` / `mongodb-org` / `mongodb-server` / `mongodb-server-core`) ; Elasticsearch (9200/tcp, `elasticsearch`) ; Memcached (11211/tcp + 11211/udp, `memcached`) ; le registre passe de 23 à **28 services** ; entrées locale `service_risk` ajoutées pour les 5 (EN + FR)
- **Services critiques/élevés installés mais inactifs** (`ufw_audit/checks/services.py`, `ufw_audit/runner.py`, `ufw_audit/display.py`) — `_check_single_service` : état `INACTIVE_DISABLED` sur service `is_high_or_critical` émet `result.warn("services.state.installed_inactive_critical")` au lieu de `result.info("services.state.inactive_disabled")` (early return conservé — pas de bruit d'exposition de port) ; `_check_port_exposure` : exposition `NOT_LISTENING` sur service high/critical émet `result.warn()` au lieu de `result.info()` ; `runner.py` : condition contexte de risque étendue de `snap.is_active` à `snap.is_active or (snap.installed and not snap.is_active and snap.service.is_high_or_critical)` ; `display.py` `build_risk_context_entries` mis à jour avec le même garde ; `services.state.installed_inactive_critical` ajouté en.json / fr.json

### Correctifs & améliorations

- **Contexte UFW inactif** (`runner.py`) — quand UFW est inactif : la section services affiche un bandeau INFO `ufw_inactive_context` explicite ; ports NetBIOS rétrogradés de WARN en INFO ; exposition des services correctement évaluée sur la couche sous-jacente ; vérification IPv6 rétrogradée en INFO ; message "tous les ports en écoute couverts" supprimé
- **Passage qualité `iptables_nftables.py`** — `nft add chain` corrigé en `nft chain` en deux endroits (ligne INPUT 183, FORWARD 246) ; regex conntrack (chemin nftables) durci de `established` à `ct\s+state\s+[^\n]*\bestablished\b[^\n]*\baccept\b`, ajout d'une exigence ACCEPT explicite pour éviter les faux positifs sur les règles DROP ; chemin iptables (`_ipt_has_conntrack`) vérifie uniquement le pattern ESTABLISHED — exigence ACCEPT à ajouter en v1.25.0 ; état FORWARD `unknown` géré avec `result.info(key="iptables_nft.forward_unknown")` au lieu d'être ignoré silencieusement ; assertions cmd dans les tests changées en `startswith` pour une validation indépendante du backend
- **Disponibilité de mise à jour noyau via apt** (`checks/kernel_modules.py`) — `_query_apt_kernel_update() -> Tuple[bool, bool, str]` retourne `(checked, update_available, candidate)` ; chemin principal : `apt-cache policy linux-image-generic` (Ubuntu/Mint, méta-paquet) ; repli : `apt list --upgradable` avec recherche de lignes `linux-image-\d` (Debian, pas de méta-paquet) ; ✔ [OK] émis quand `apt_checked=True` et `apt_update_available=False` ; champs `apt_update_available`, `apt_candidate_kernel`, `apt_checked` ajoutés à `KernelModulesSnapshot` ; bloc apt placé avant l'early return `len(kernels) <= 1` pour que les systèmes mono-noyau reçoivent aussi le résultat

### Tests

| Fichier | Classe | Modification |
|---------|--------|-------------|
| `tests/test_iptables_nftables.py` | nouveau fichier | 51 tests — `TestIPTablesNFTablesSnapshot`, `TestCheckIPTablesNFTables` (politiques INPUT/FORWARD, conntrack, backend nftables), `TestForwardPolicy` (accept + forward), `TestConntrackDetection` (cas limites regex), `TestNFTablesBackend` (cmd nft chain), `TestForwardUnknown` (niveau INFO) |
| `tests/test_kernel_modules.py` | `TestKernelAptUpdate` | +9 tests — mise à jour disponible, à jour ✔ quand apt consulté, pas de ✔ si apt non consulté, candidat dans le message |
| `tests/test_services.py` | `TestInactiveDisabled` | +5 tests — `test_warn_for_critical_inactive_disabled`, `test_warn_for_high_inactive_disabled`, `test_no_deduction_for_critical_inactive`, `test_no_port_check_for_critical_inactive` ; `test_info_finding_for_inactive` mis à jour avec `risk="low"` |
| `tests/test_services.py` | `TestPortExposureFindings` | +4 tests — `test_not_listening_critical_adds_warn`, `test_not_listening_high_adds_warn`, `test_not_listening_critical_no_deduction` ; `test_not_listening_adds_info` mis à jour avec `risk="low"` |
...
- **`checks/ports.py`** : signature de `_split_addr_port()` passée de `(addr, port)` à `(addr, port, iface)` — le suffixe de scope d'interface (ex. `%virbr0`) est maintenant capturé séparément ; `ListeningPort` reçoit le champ `iface: str = ""` ; `is_all_interfaces` retourne `False` quand `iface` est défini — corrige `0.0.0.0%virbr0:67` (dnsmasq lié au bridge KVM virbr0) traité comme port toutes-interfaces dans le tableau d'exposition
- **`exposure.py`** : ports UDP au-dessus de 32767 exclus de l'ensemble des ports exposés dans `compute_exposure()` — reflète le filtre `PortCategory.EPHEMERAL` de `check_ports()` ; les sockets éphémères avahi/mDNS (ex. 37238/udp, 52289/udp) n'apparaissent plus dans la surface d'attaque

### Fonctionnalité

- **`runner.py`** : sortie de `ufw status verbose` affichée après les findings règles UFW en mode verbeux (`-v`) — fournit le contexte des règles immédiatement sans appel séparé à `sudo ufw status verbose`

### Tests

| Fichier | Changement | Détail |
|---------|------------|--------|
| `tests/test_ports.py` | +2 tests | `TestSplitAddrPort` : toutes les assertions mises à jour pour le retour 3-tuple ; `test_ipv4_virbr0_iface` ajouté ; `TestListeningPort` : `test_is_all_interfaces_false_when_iface_scoped` ajouté |
| `tests/test_exposure.py` | 1 renommé + 1 ajouté (55 au total) | `test_high_numbered_listen_port_is_shown` → `test_high_numbered_tcp_port_is_shown` ; `test_high_numbered_udp_port_excluded` vérifie que UDP > 32767 n'est PAS affiché |
...
### Résumé
- **SSH `PermitRootLogin` cas OK** — `no` → OK ; `prohibit-password`/`forced-commands-only` → OK restreint ; `yes` → ALERT −3 pt (inchangé) ; valeur inconnue → INFO
- **Performance scan SUID** — `find /` → liste ciblée `_SCAN_ROOTS` ; durée <1 s ; liste blanche SGID : `camel-lock-helper-1.2`, `support-tool-launcher`
- **Correctif affichage dominance IoT** — le finding INFO `local_dominance` était silencieusement ignoré ; corrigé par vérification de clé explicite dans `display_log_results`
- **i18n labels scores par domaine** — `render_domain_scores` passe les labels par `t()` ; `samba`/`disk` ajoutés aux locales
- **6 nouveaux contrôles sysctl durcissement** — tcp_syncookies, accept_source_route, accept_redirects_v6, send_redirects, protected_hardlinks, protected_symlinks — tous WARN −1 pt, tous lus via `/proc/sys/`

### Audit SSH (`checks/ssh.py`)

- `_check_sshd_config` : vérification `PermitRootLogin` désormais complète
  - `yes` → `result.alert()` + `add_deduction(points=3)` (inchangé)
  - `no` → `result.ok(key="ssh.permit_root_login_disabled")`
  - `prohibit-password` ou `forced-commands-only` → `result.ok(key="ssh.permit_root_login_restricted", value=prl)`
  - valeur inconnue → `result.info(key="ssh.permit_root_login", value=prl)`
- Valeur par défaut supposée : `prohibit-password` (défaut SSH)
...
### Scan SUID/SGID (`checks/suid_audit.py`)

- Tuple `_SCAN_ROOTS` remplace le `find /` complet : `/bin`, `/sbin`, `/usr/bin`, `/usr/sbin`, `/usr/local/bin`, `/usr/local/sbin`, `/usr/local/lib`, `/usr/lib`, `/usr/lib64`, `/lib`, `/lib64`, `/usr/libexec`, `/opt`
- `from_system()` filtre `_SCAN_ROOTS` aux répertoires existants ; construit la commande `find` avec filtre de permission SUID+SGID combiné
- `_KNOWN_SGID` étendu : `"camel-lock-helper-1.2"`, `"support-tool-launcher"`
- Timeout maintenu à 15 s (`_FIND_TIMEOUT`)

### Correctif affichage (`display.py`)

- `display_log_results` n'itérait que les findings WARN ; `logs.local_dominance` (INFO) était silencieusement ignoré
- Corrigé : après la boucle top-ports, itération de tous les findings INFO ; affichage de ceux avec `key == "logs.local_dominance"`

### i18n scores par domaine (`domain_scores.py`)

- `render_domain_scores(scores, t=None)` — helper interne `_label(domain, fallback)` ajouté
- `_label` appelle `t(f"domain_scores.{domain}")` ; retourne le repli anglais si la clé n'est pas encore traduite
- Dict `labels` construit par domaine ; `label_width` dérivé des labels traduits
- Nouvelles clés locale : `domain_scores.samba` (EN : "Samba Security" / FR : "Sécurité Samba"), `domain_scores.disk` (EN : "Disk Health" / FR : "Santé des disques")

### Contrôles sysctl durcissement (`checks/hardening.py`)

Nouveaux champs dans `HardeningSnapshot` (tous lus via `_read_sysctl_bool` / `_read_sysctl_int` depuis `/proc/sys/`) :

| Champ | Clé | Défaut | Condition OK | Déduction |
|-------|-----|--------|--------------|-----------|
| `tcp_syncookies` | `net.ipv4.tcp_syncookies` | 1 | ≥ 1 | −1 pt si 0 |
| `accept_source_route` | `net.ipv4.conf.all.accept_source_route` | False | False | −1 pt si True |
| `accept_redirects_v6` | `net.ipv6.conf.all.accept_redirects` | False | False | −1 pt si True |
| `send_redirects` | `net.ipv4.conf.all.send_redirects` | False | False | −1 pt si True |
| `protected_hardlinks` | `fs.protected_hardlinks` | True | True | −1 pt si False |
| `protected_symlinks` | `fs.protected_symlinks` | True | True | −1 pt si False |

Toutes les commandes de correction ciblent `/etc/sysctl.d/99-hardening.conf`.

### Tests

- `tests/test_hardening.py` : `make_snapshot` gagne 6 nouveaux défauts ; `TestTcpSyncookies` (6), `TestAcceptSourceRoute` (5), `TestAcceptRedirectsV6` (5), `TestSendRedirects` (5), `TestProtectedHardlinks` (4), `TestProtectedSymlinks` (4) — 29 nouveaux tests
- `tests/test_ssh.py` : 7 nouveaux tests `PermitRootLogin`
- **Total : 3259/3259 (+530 par rapport à v1.18.0)**
...
### Résumé
- **CHECK 31** — Linux Audit Framework (auditd) : installation, service actif, règles chargées, fichiers sensibles surveillés → WARN −1 pt chacun pour service inactif / aucune règle / fichiers non couverts (profil server)
- **CHECK 32** — Secure Boot : détection mokutil/efivars/bootctl → WARN −1 pt si désactivé sur desktop ; INFO si désactivé sur server/VM ou BIOS/inconnu
- **CHECK 33** — Intégrité des fichiers (AIDE/Tripwire) : installé, base initialisée, dernier check ≤30 jours → WARN −1 pt chacun
- **Variantes `--explain` par profil** : 17 clés affichent des sections `[ server ]` / `[ desktop ]` / `[ container ]` ; note uniforme jaune pour les clés sans différence entre profils ; délai ESC réduit à 25 ms
- **Profil `workstation` → `desktop`** : renommé ; alias `workstation` conservé pour la rétrocompatibilité
- **`cmd_type`** : `Finding` gagne `cmd_type: str = "fix"` / `"check"` ; préfixe différent dans la boîte de synthèse
- **Correctifs** : fausse alerte IPv6 avahi, repli journald (Debian 13), persistance sysctl hardening, hint smartmontools
- **UX** : services triés par gravité, panorama alphabétique, profil dans la boîte de synthèse, couleurs contexte de risque, titre de groupe centré
- **Trusted Publishing** : `.github/workflows/publish.yml` — OIDC → PyPI ; aucun token API
- **2507/2507 tests** (+215 vs v1.16.0)

### Modifications

**`checks/auditd.py`** (CHECK 31 — nouveau fichier)
- `AuditdSnapshot` : `installed: bool`, `service_active: bool`, `rules_loaded: int`, `watched_files: list[str]`
- `from_system()` : `shutil.which("auditctl")` → installed ; `systemctl is-active auditd` → service_active ; `auditctl -l` → comptage des règles et extraction des chemins surveillés ; cibles sensibles : `/etc/passwd`, `/etc/shadow`, `/etc/sudoers`
- `check_auditd()` : non installé → INFO ; service inactif → WARN −1 pt `auditd.service_inactive` ; aucune règle → WARN −1 pt `auditd.no_rules` ; fichiers sensibles non couverts → WARN −1 pt (server) / INFO (desktop) `auditd.missing_watches` ; tout OK → OK `auditd.ok`

**`checks/secure_boot.py`** (CHECK 32 — nouveau fichier)
- `SecureBootSnapshot` : `state: str` ("enabled" / "disabled" / "no_uefi" / "unknown"), `method: str`
- Priorité de détection : `mokutil --sb-state` → lecture octet `/sys/firmware/efi/efivars/SecureBoot-*` → grep `bootctl status`
- `check_secure_boot()` : activé → OK ; désactivé sur desktop → WARN −1 pt `secure_boot.disabled` ; désactivé sur server → INFO ; no_uefi / inconnu → INFO

**`checks/file_integrity.py`** (CHECK 33 — nouveau fichier)
- Constantes : `_CHECK_WARN_DAYS = 30` ; chemins BDD AIDE (`/var/lib/aide/aide.db*`) ; répertoire BDD + logs Tripwire
- `FileIntegritySnapshot` : `tool: str` ("aide" / "tripwire" / ""), `db_exists: bool`, `last_check_date: Optional[str]`
- `from_system()` : AIDE détecté en priorité ; présence de la BDD via liste de chemins ; date du dernier check via mtime du fichier log ; Tripwire : fichier `.txt` le plus récent dans le répertoire de logs
- `_check_age_days(iso_date)` : UTC-aware — `datetime.strptime(...).replace(tzinfo=timezone.utc)` + `datetime.now(timezone.utc)`
- `check_file_integrity()` : non installé → INFO ; BDD absente → WARN −1 pt ; aucun check → WARN −1 pt ; check > 30 jours → WARN −1 pt ; OK si check récent

**`explain.py`**
- `_EXPLAIN_PROFILES = ("server", "desktop", "container")`
- `_has_profile_variants(key, t)` : sonde `t(f"explain.{key}.server.why")` ; vérifie les formes brute et `[entre crochets]` (gère i18n réel vs mock t())
- `run_explain()` : si variantes → affiche `[ profil ]` + diviseur + WHY et HOW du profil (spécifique ou repli générique) ; sinon → WHY/HOW générique + note uniforme jaune (`\033[33m...\033[0m` si `sys.stdout.isatty()`)
- `_detail_screen._build_lines()` : même logique en mode curses ; note uniforme via `color_pair(2)` (COLOR_YELLOW)
- `os.environ.setdefault("ESCDELAY", "25")` avant l'appel à `curses.wrapper()`

**`locales/en.json` + `fr.json`**
- Variantes par profil pour 17 clés : `updates.unattended_not_configured`, `updates.security_pending`, `hardening.rp_filter_disabled`, `hardening.rp_filter_loose`, `hardening.redirects_enabled`, `hardening.log_martians_disabled`, `ipv6.port_no_v6_rule`, `ipv6.ufw_disabled_listeners_present`, `memory.swappiness_ssd_wear`, `memory.swappiness_unjustified`, `services_state.enabled_inactive`, `clamav.db_very_outdated`, `clamav.db_outdated`, `clamav.scan_very_old`, `clamav.scan_old`, `kernel_modules.risky_fs`, `kernel_modules.risky_net`
- Structure par clé : `explain.KEY.server.why`, `explain.KEY.desktop.why`, `explain.KEY.container.why` ; `explain.KEY.container.how` optionnel pour les clés sysctl/noyau/mémoire
- Nouvelles sections `auditd`, `secure_boot`, `file_integrity` dans les deux langues

**`scoring.py`**
- `Finding` : champ `cmd_type: str = "fix"` ; propagé dans toutes les méthodes `result.ok/warn/alert/info()`

**`output.py`**
- `print_check_cmd()` : affiche `ℹ Vérifier : cmd` (vs `→ cmd` pour les corrections)
- `print_group()` : titre du groupe centré dans la ligne `━` pleine largeur
- `orange` + `orange_bold` ajoutés à `_Colours`
- `print_risk_context()` : paramètre `risk_tier` — rouge=critique, orange=moyen, jaune=faible

**`checks/ipv6.py`**
- `ss -tuln` → `ss -tulnp` pour capturer les noms de processus
- `_INTERNAL_PROCESSES` frozenset : avahi-daemon, systemd-resolve, dnsmasq, containerd, dockerd
- `_extract_ipv6_listeners()` : filtre les processus internes ; plus de suggestions de règles UFW pour avahi/cups/loopback
- Warning `ufw_disabled_listeners_present` : `detail=t("ipv6.listeners_list", ports=...)` affichant les ports concernés

**`checks/logs.py`**
- `LogsSnapshot` : nouveau champ `log_source: str` ("file" / "journald" / "none")
- `from_system()` : bascule sur `_read_from_journald(log_days)` quand `/var/log/ufw.log` absent ; `journalctl -k --output=short-iso`
- `check_logs()` : INFO discret si `source=journald` ; message `no_logfile` mis à jour (mentionne journald)

**`checks/hardening.py`**
- `rp_filter_disabled` / `rp_filter_loose` / `redirects_enabled` / `log_martians_disabled` : commandes de correction persistantes via `sysctl -w ... && echo ... >> /etc/sysctl.d/99-hardening.conf`

**`checks/disk.py`**
- Finding `disk.smartctl_missing` : `cmd="sudo apt install smartmontools"` ajouté

**`manage_logs.py`**
- `run_manage_logs()` refactorisé avec `while True` — menu reste ouvert après chaque suppression ; liste des logs rafraîchie à chaque itération ; quitter : Entrée ou `q`

**`runner.py`**
- `_snap_severity()` : CRITIQUE(0) → ÉLEVÉ(1) → MOYEN(2) → FAIBLE(3) → inactif(4)
- `snapshots = sorted(snapshots, key=_snap_severity)` avant la boucle d'affichage
- CHECK 31 inséré en tête du GROUPE 5 (DÉTECTION & SANTÉ) ; CHECK 32 avant CHECK 29 ; CHECK 33 après CHECK 30

**`panorama.py`**
- `build_panorama_rows()` : `return sorted(rows, key=lambda r: r["label"].lower())`

**`display.py`**
- `print_audit_summary()` : paramètre `profile_name: str = "server"` ; ligne du profil ajoutée dans la boîte de synthèse
- `display_risk_context()` : détecte le niveau de risque du texte traduit → transmis à `print_risk_context()`

**`profiles.py`**
- `load_profile()` : alias `workstation` → `desktop`

**`data/profiles/desktop.conf`** (nouveau fichier)
- Mêmes surcharges que l'ancien `workstation.conf` ; `container.conf` mis à jour avec `extends = desktop`

**`.github/workflows/publish.yml`** (nouveau fichier)
- Workflow Trusted Publishing : déclenché sur les tags `v*` ; jobs : test → build → publish
- `environment: release` ; permission `id-token: write` ; `pypa/gh-action-pypi-publish@release/v1`

### Tests

| Fichier | Modification | Couverture |
|---------|-------------|------------|
| `tests/test_auditd.py` | Nouveau — 41 tests | Défauts `AuditdSnapshot` ; chemins `from_system()` ; `check_auditd()` — non installé, inactif, sans règles, watches manquants (server/desktop), tout OK |
| `tests/test_secure_boot.py` | Nouveau — 21 tests | Défauts `SecureBootSnapshot` ; méthodes de détection ; `check_secure_boot()` — activé, désactivé desktop (WARN), désactivé server (INFO), no_uefi (INFO), inconnu (INFO) |
| `tests/test_file_integrity.py` | Nouveau — 33 tests | Défauts `FileIntegritySnapshot` ; cas limites `_check_age_days()` ; `check_file_integrity()` — non installé, BDD absente (aide/tripwire), sans check, check ancien, propre ; repli outil inconnu ; `TestEdgeCases` (date invalide, priorité, outil inconnu) |
| `tests/test_explain.py` | +81 tests | `TestHasProfileVariants` (12) ; `TestRunExplainProfileVariants` (24 × sections par profil) ; `TestRunExplainUniform` (note jaune, pas de sections) ; branchement dans `test_known_key_shows_why_and_how_headers` |
...
### Résumé
- **CHECK 19** — Détection d'applications de bureau : 30+ applis GUI (Steam, Discord, Zoom, Signal, RustDesk, Wireshark, Betterbird, virt-manager…) en cours d'exécution → INFO, sans déduction
- **CHECK 28** — Synchronisation NTP : systemd-timesyncd/chronyd/ntpd actif et synchronisé → WARN −1 pt si non
- **CHECK 29** — Prévention d'intrusion Fail2ban (autonome) : état service, jails actifs, jail SSH → WARN −1 pt si inactif ou sans jails
- **CHECK 30** — Scan rootkit & intégrité : fraîcheur BDD rkhunter/chkrootkit et date dernier scan → WARN −1 pt chaque
- **`--target N` code de sortie 4** — retourne `EXIT_TARGET_MISSED = 4` quand score < cible ; prioritaire sur les codes 1/2
- **Validation CLI** — `--explain=`, `--profile=`, `--lang=`, `--webhook=`, `--target=` avec valeur vide lèvent une `CLIError`
- **5 en-têtes de groupes thématiques** — sortie réorganisée en 5 groupes ; `print_group()` avec séparateur `━` cyan pleine largeur
- **Fail2ban retiré du durcissement** — check autonome dédié dans le GROUPE 5 (DÉTECTION & SANTÉ)
- **Passage qualité** — `desktop_apps.py` : clés `_KNOWN_APPS` en minuscules, doublon supprimé, lookup simplifié
- **2292/2292 tests** (+153 vs v1.15.1)

### Modifications

**`checks/desktop_apps.py`** (CHECK 19 — nouveau fichier)
- `_KNOWN_APPS` : dict `{comm_minuscule: nom_affichage}` pour 30+ applications GUI connues réparties en 7 catégories : gaming (Steam), navigateurs (Firefox, Brave, Chromium), communication (Discord, Zoom, Teams, Slack, Skype, Telegram, Signal, WhatSie, Element, Nheko, Fractal), messagerie (Betterbird, Thunderbird), sync fichiers (kDrive), médias (Spotify, VLC, OBS, Transmission), accès distant/sécurité (RustDesk, Wireshark), virtualisation (virt-manager) ; toutes les clés ≤ 15 chars (troncature `comm` Linux)
- `DesktopAppsSnapshot` : `detected: List[Tuple[str, str]]` (nom d'affichage, nom du processus)
- `DesktopAppsSnapshot.from_system()` : lit la sortie de `ps -eo comm` ; déduplique par nom d'affichage (plusieurs noms de processus peuvent pointer vers la même appli)
- `check_desktop_apps()` : aucune appli → résultat vide (section non émise) ; appli détectée → finding INFO ; sans déduction

**`checks/ntp.py`** (CHECK 28 — nouveau fichier)
- `NtpSnapshot` : `ntp_service`, `ntp_active`, `ntp_synced`
- `NtpSnapshot.from_system()` : détecte systemd-timesyncd/chronyd/ntpd via `systemctl is-active` ; parse `timedatectl show` clé=valeur pour `NTP=yes` et `NTPSynchronized=yes`
- `check_ntp()` : non installé → INFO ; actif+synchronisé → OK ; actif+non synchronisé → WARN −1 pt `ntp.not_synced` ; inactif → WARN −1 pt `ntp.inactive`

**`checks/fail2ban.py`** (CHECK 29 — nouveau fichier)
- `_SSH_JAIL_PATTERNS = ("sshd", "ssh")`
- `Fail2banSnapshot` : `installed`, `service_active`, `active_jails`, `ssh_jail`
- `Fail2banSnapshot.from_system()` : `shutil.which("fail2ban-client")` → installed ; `systemctl is-active` → service_active ; repli : `fail2ban-client ping` → "pong" ; `fail2ban-client status` → `_parse_jails()` ; jail SSH via `next(generator, "")`
- `_parse_jails(status_output)` : cherche la ligne "Jail list:" (insensible à la casse), découpe sur `,`
- `check_fail2ban()` : non installé → INFO ; inactif → WARN −1 pt `fail2ban.service_inactive` ; sans jails → WARN −1 pt `fail2ban.no_jails` ; jails actifs → OK `fail2ban.active` ; jail SSH → OK supplémentaire `fail2ban.ssh_jail_active`

**`checks/rootkit.py`** (CHECK 30 — nouveau fichier)
- Constantes : `_DB_WARN_DAYS = 7`, `_SCAN_WARN_DAYS = 30`
- `RootkitSnapshot` : `rkhunter_installed`, `chkrootkit_installed`, `tool`, `db_age_days`, `last_scan_date`
- `RootkitSnapshot.from_system()` : `shutil.which()` pour les deux outils ; âge BDD rkhunter via mtime de `_RKHUNTER_DB` ; date dernier scan : lit les 100 derniers Ko de `/var/log/rkhunter.log` (un scan complet génère 130 Ko+), confirme la présence d'un scan terminé via `_RKHUNTER_SCAN_RE`, retourne le mtime du fichier comme date (robuste face à rkhunter 1.4.x qui ne journalise que `[HH:MM:SS]`, sans date sur les lignes de contenu) ; chkrootkit via mtime de `/var/log/chkrootkit/log.today`
- `check_rootkit()` : aucun outil → INFO ; BDD obsolète → WARN −1 pt `rootkit.db_outdated` ; pas de scan → WARN −1 pt `rootkit.no_scan` ; scan trop ancien → WARN −1 pt `rootkit.scan_old` ; tout OK → OK `rootkit.ok`

**`checks/hardening.py`**
- Supprimé : champ `fail2ban_active: bool` dans `HardeningSnapshot`
- Supprimé : bloc de détection fail2ban dans `from_system()`
- Supprimé : bloc de vérification fail2ban dans `check_hardening()` (clés `hardening.fail2ban_ok` / `hardening.fail2ban_missing`)

**`__main__.py`**
- Constante `EXIT_TARGET_MISSED = 4` ajoutée
- Logique de sortie : `if config.target > 0 and engine.score < config.target: return EXIT_TARGET_MISSED` — vérifié avant `EXIT_ALERTS`/`EXIT_WARNINGS`

**`output.py`**
- `print_group(title)` : affiche une barre `━` pleine largeur + titre en gras cyan ; appelé une fois par groupe thématique dans `runner.py`

**`report.py`** (`AuditReport`)
- `write_group(title)` : écrit un bloc séparateur `=== TITRE ===` dans le fichier rapport

**`report_markdown.py`** (`MarkdownReport`)
- `write_group(title)` : écrit un titre Markdown `# TITRE` (pour la cohérence)

**`runner.py`**
- Tous les checks réorganisés en 5 groupes thématiques avec appels `print_group()` + `report.write_group()` :
  - GROUPE 1 — FIREWALL & RÉSEAU : statut firewall, règles, IPv6, ports, logs, DDNS, docker, virtualisation
  - GROUPE 2 — EXPOSITION & SERVICES : services, stack firewall, contexte réseau
  - GROUPE 3 — CONTRÔLE D'ACCÈS : SSH, permissions fichiers, comptes utilisateurs, politique mots de passe
  - GROUPE 4 — DURCISSEMENT SYSTÈME : durcissement, mises à jour, modules noyau, cron, état services, disque, mémoire, Samba, ClamAV, SMTP, dominance IoT
  - GROUPE 5 — DÉTECTION & SANTÉ : applications de bureau, NTP, fail2ban, rootkit
- Ajout des imports `Fail2banSnapshot`, `check_fail2ban`, `RootkitSnapshot`, `check_rootkit`, `print_group`

**Locales (`en.json` + `fr.json`)**
- Ajouté : section `"groups"` avec 5 clés (`groups.firewall_network`, `groups.exposure_services`, `groups.access_control`, `groups.system_hardening`, `groups.detection_health`)
- Ajouté : `"sections.ntp"`, `"sections.fail2ban"`, `"sections.rootkit"`, `"sections.desktop_apps"`
- Ajouté : `"ntp.*"` (4 clés), `"fail2ban.*"` (5 clés), `"rootkit.*"` (4 clés), `"desktop_apps.*"` (3 clés)
- Supprimé : `hardening.fail2ban_ok`, `hardening.fail2ban_missing`

**`explain.py`**
- Supprimé : `"hardening.fail2ban_missing"` du groupe Hardening dans `_EXPLAIN_GROUPS`
- Nombre de clés : 77 → 76 ; groupes : 17 → 19

**Tests**
- `test_desktop_apps.py` : nouveau — défauts snapshot, clés `_KNOWN_APPS`, `from_system()`, toutes les branches `check_desktop_apps()`
- `test_ntp.py` : nouveau — défauts snapshot, parsing `timedatectl`, détection service, toutes les branches `check_ntp()`
- `test_fail2ban.py` : nouveau — 42 tests ; `_parse_jails()`, snapshot, chemins `from_system()`, toutes les branches `check_fail2ban()`
- `test_rootkit.py` : nouveau — 38 tests ; snapshot, deux outils, toutes les branches `check_rootkit()`
- `test_exit_codes.py` : nouveau — 18 tests ; constantes, logique de priorité `_decide_exit()`
- `test_hardening.py` : `TestFail2ban` supprimé (−3) ; tests composites avec `log_martians=False`
- `test_explain.py` : nombre de clés 77→76 ; `hardening.fail2ban_missing` supprimé
- **2292 tests** (tous passants)
...
### Résumé
- **CHECK 26** — Dominance source locale IoT dans les logs UFW (≥ 70 % du trafic bloqué depuis une seule IP privée)
- **CHECK 27** — Exposition SMTP locale (Postfix/Exim en écoute sur 0.0.0.0:25 vs 127.0.0.1:25)
- **C1** — `--fix` aperçu par défaut ; `--fix --apply` pour exécuter ; `--fix --apply --yes` confirmation automatique
- **C2** — `--target N` objectif de score : affiche l'écart ou la réussite dans la boîte de synthèse
- **Corrections TUI `--explain`** — navigation bloquée, écran détail in-curses, comportement ESC/q corrigé, en-têtes restaurés au défilement
- **`--explain` 73→77 clés** — groupe `user_accounts` (+4 clés)
- **`q` annule les assistants** — `--install-cron`, `--manage-cron`, `--manage-logs`
- **Passage qualité `smtp.py`** — bug wildcard `*`, pire cas multi-bind, suppression crochets IPv6, commande correctif Postfix-spécifique
- **2139/2139 tests** (+93 vs v1.14.0)

### Modifications

**`checks/logs.py`** (CHECK 26)
- `_dominant_local_source(entries)` : compte les entrées bloquées par IP source privée ; retourne `(ip, count, pct)` quand une IP représente ≥ 70 % et le total ≥ 50
- `check_logs()` : appelle `_dominant_local_source()` après les vérifications bruteforce/services ; émet `logs.local_dominance` WARN −1 pt
- Constantes `_LOCAL_DOMINANCE_THRESHOLD = 0.70`, `_LOCAL_DOMINANCE_MIN_COUNT = 50`

**`checks/smtp.py`** (CHECK 27 — nouveau fichier)
- `SmtpSnapshot` : `installed`, `mta_name`, `listening`, `bind_address`, `exposed`
- `SmtpSnapshot.from_system()` : détection MTA via `ps -eo comm` ; vérification port 25 via `_check_port_25()` (ss → netstat)
- `_LOCAL_BIND_RE` : correspond à `127.*`, `::1`, `localhost` comme adresses sûres
- `check_smtp()` : non installé → OK ; installé non en écoute → INFO ; local uniquement → INFO ; exposé → WARN −1 pt

**`cli.py`**
- Nouveau champ `apply: bool = False` dans `AuditConfig` ; `--apply` analyse ; requiert `--fix`
- `--yes` requiert désormais `--fix --apply` (était `--fix`)
- Incompatibilité `--quiet`/`--json` limitée à `--fix --apply` (mode aperçu compatible)
- Nouveau champ `target: int = 0` ; `--target=N` / `--target N` (1–10)

**`fixes.py`**
- Branche aperçu : `if not getattr(config, "apply", False):` — affiche `fixes.dry_run_hint` + `→ cmd` pour chaque correction, retourne sans exécuter

**`display.py`**
- `print_audit_summary()` : lit `config.target` ; ajoute une ligne objectif quand défini ; `✔` vert si atteint, `▲` jaune avec écart sinon

**`explain.py`**
- `_picker()` : navigation bloquée (sans rebouclage) ; Entrée appelle `_detail_screen()` ; seuls `q`/`Q` quittent
- `_detail_screen()` : écran de détail in-curses ; défilable ↑↓/PgUp/PgDn ; ESC seul ferme
- En-têtes de groupe restaurés lors du défilement vers le haut
- Groupe `user_accounts` ajouté : `uid_zero`, `empty_password`, `expired_account`, `no_shadow`

**`cron.py`** / **`manage_logs.py`**
- `q` annule à n'importe quelle étape de `--install-cron`, `--manage-cron`, `--manage-logs`
- `prompt_emails()` : `q. Annuler — retour au menu` affiché comme entrée explicite dans la liste afin que l'utilisateur sache qu'il peut quitter sans rien modifier (clé `email_prompt.cancel`) ; comportement inchangé — `q` retournait déjà `None`
- `--manage-cron` affiche une ligne par adresse email dans la liste

**Locales (`en.json` + `fr.json`)**
- Ajouté : `logs.local_dominance`, `smtp.*` (5 clés), `fixes.dry_run_hint`
- Ajouté : `scoring.target_label`, `scoring.target_reached`, `scoring.target_gap`
- Ajouté : `install_cron.quit_hint`, `install_cron.cancelled`, `manage_cron.cancelled`, `manage_logs.cancelled`
- Ajouté : `email_prompt.cancel`

**Autocomplétion bash**
- Ajout de `--apply`, `--target=`
...
- Tests mis à jour : `test_cli.py` pour la nouvelle sémantique `--apply`
- **2130 tests** (tous réussis)

**Passage qualité — `checks/smtp.py`**
- `_LOCAL_BIND_RE` : suppression de `\*$` — `*` dans `ss` signifie toutes les interfaces (classé à tort comme local, produisant des faux négatifs sur les systèmes avec `*:25`)
- `_check_port_25()` : collecte tous les binds du port 25 en une passe ; supprime les crochets des adresses IPv6 avant le matching (`[::1]` → `::1`, `[::]:25` → `::` → exposé) ; retourne l'adresse la plus exposée en cas de multi-bind (`127.0.0.1:25` + `0.0.0.0:25` → retourne `0.0.0.0`)
- `check_smtp()` : commande de correction spécifique Postfix (`sudo postconf -e 'inet_interfaces = loopback-only'`) ; champ `note` pour le hint de redémarrage ; `cmd` vide pour Exim/autres MTA (action manuelle)
- Clé `smtp.exposed_restart_postfix` ajoutée dans `en.json` + `fr.json`
- `test_smtp.py` : `test_wildcard_star` → `test_wildcard_star_is_exposed` (assertion inversée) ; +2 tests `_LOCAL_BIND_RE` ; `TestSmtpCmd` (4 tests) ; `TestSmtpWildcardExposed` (3 tests)
- **2139 tests** (tous réussis)
...
### Résumé
- **CHECK 24 — Audit sécurité Samba** (`checks/samba.py`) — SMB1, mots de passe nuls, partages invités, signature serveur, map-to-guest ; 6 findings ; nouveau domaine Samba
- **CHECK 25 — Audit antivirus ClamAV** (`checks/clamav.py`) — détection installation, fraîcheur BD, âge dernier scan ; datetime naïve (heure locale) pour l'analyse des logs
- **Correctif `--diff`** — `info_count` désormais suivi et inclus dans le delta de la baseline
- **`--explain` 63→73 clés** (17 groupes) — Samba (6) + Disque (5) ajoutés
- **2045/2045 tests** (+155 vs v1.13.0)

### Modifications

**`checks/samba.py`**
- `SambaSnapshot` + `check_samba()` — séparation snapshot/logique pure
- SMB1 activé → ALERT −2 ; mots de passe nuls → ALERT −3 ; partage invité accessible en écriture → WARN −1 ; lecture seule invité → INFO ; signature serveur désactivée → WARN −1 ; map to guest → WARN −1
- 12 clés de locale dans `en.json` + `fr.json`

**`checks/clamav.py`** (+ passage qualité)
- `ClamavSnapshot` + `check_clamav()` — installation via `clamav-daemon`, `freshclam`, ou socket clamd (`_CLAMD_SOCKETS`)
- Âge BD via `datetime.fromtimestamp(st_mtime, tz=timezone.utc)` + `.days` ; ≥14 j → ALERT −2, ≥7 j → WARN −1
- Âge dernier scan parsé depuis `/var/log/clamav/clamav.log` en **heure locale naïve** (ClamAV logue en heure locale) ; ≥30 j → ALERT −2, ≥14 j → WARN −1
- `(?i)` sur `_END_DATE_RE` ; constante `_CLAMD_SOCKETS` pour le repli socket

**`compare.py`**
- `build_baseline()` inclut désormais `info_count` ; `compute_delta()` signale les changements dans la sortie diff

**`explain.py`**
- 73 clés sur 17 groupes ; ajout Samba (6) et Disque (5)

**Tests**
- `tests/test_samba.py` : 52 tests
- `tests/test_clamav.py` : 52 tests ; datetime naïve pour l'âge du scan ; tests repli freshclam-only et socket clamd
...
### Résumé
- **CHECK 22 — Santé disques** (`checks/disk.py`) — Santé SMART (`smartctl -H/-A`) + utilisation partitions (`df -P`) ; nouveau domaine `disk` (6e) ; 22 clés de locale ; support NVMe ; conseils SMART
- **CHECK 23 — Mémoire & Swap** (`checks/memory.py`) — Détection usure SSD, swap injustifié (3 conditions), swappiness adapté au profil ; domaine `hardening` ; 9 clés de locale
- **Tableau partitions** — `display_disk_partitions()` avec barres de progression colorées (vert/jaune/rouge) dans la section ÉTAT DES DISQUES
- **`--explain` 33 → 63 clés** — 30 nouvelles clés dans 15 groupes ; `--explain list` affiche les en-têtes de groupes
- **Passage qualité** — disk.py (NVMe, parsing SMART ciblé) ; memory.py (swap 3 conditions, swapon --show=NAME, /proc errors="ignore")
- **Tests : 1890/1890** (+187 : `test_disk.py` 60 tests, `test_memory.py` 37 tests)

### CHECK 22 — Santé disques (`checks/disk.py`)

**Snapshot** (`DiskSnapshot`) :
- `smartctl_available` : bool — True si `smartctl` est disponible sur le PATH
- `smart_results` : List[SmartResult] — un élément par disque physique (`lsblk -d -n -o NAME,TYPE`)
- `partitions` : List[PartitionInfo] — systèmes de fichiers montés non-pseudo (`df -P --block-size=1G`)

**Champs SmartResult** : `device`, `model`, `passed` (None = inconnu), `virtual` (VM/non supporté), `reallocated_sectors`, `pending_sectors`, `uncorrectable_errors`

**Logique de vérification** (`check_disk`) :
| Condition | Niveau | Déduction |
|-----------|--------|-----------|
| SMART FAILED | ALERT | −3 pts |
| Secteurs réalloués > 0 | WARN | −1 pt |
| Secteurs en attente > 0 | WARN | −1 pt |
| Erreurs non corrigibles > 0 | WARN | −1 pt |
| Partition ≥ 90% pleine | WARN | −1 pt |
| Partition ≥ 80% pleine | INFO | aucune |
| smartctl absent | INFO | aucune |
| Appareil virtuel/non supporté | INFO | aucune |

**Helpers** :
- `_detect_block_devices()` : `lsblk -d -n -o NAME,TYPE` → `/dev/{name}` pour type=disk
- `_query_smart(device)` : `smartctl -iH` (info + santé) + `smartctl -A` (attributs) ; détecte virtuel/non supporté par mots-clés ; santé SMART matchée uniquement sur la ligne "SMART overall-health"
- `_parse_nvme_attrs(output)` : mappe les compteurs NVMe — Media and Data Integrity Errors → `uncorrectable_errors`, Error Information Log Entries → `pending_sectors`
- `_parse_smart_attr(output, attr_id)` : parse RAW_VALUE à l'index 9 (format standard `smartctl -A`) ; gère la notation parenthétique
- `_read_partition_usage()` : ignore tmpfs, devtmpfs, squashfs, overlay, udev, cgroupfs, proc, sysfs, etc. ; `--block-size=1G` pour l'affichage en Go
- Finding INFO `disk.smart_tips` : `smartctl -a` par disque + commandes guidées (test court/long, watch, abort `-X`, historique `-l selftest`)

**Nouveau domaine `disk`** dans `domain_scores.py` :
- Ajouté en 6e position dans la liste `DOMAINS` (entre `hardening` et `firewall`)
...
- `mem_total_kb`, `mem_available_kb` : depuis `/proc/meminfo` (avec `errors="ignore"`)
- `swap_total_kb`, `swap_free_kb` : depuis `/proc/meminfo`
- `swappiness` : depuis `/proc/sys/vm/swappiness` (avec `errors="ignore"`)
- `swap_on_ssd` : bool — `/sys/block/{dev}/queue/rotational` = 0 ; gère le stripping des partitions NVMe
- `swap_devices` : List[str] — depuis `swapon --show=NAME --noheadings --raw`

**Logique de vérification** (`check_memory`, `profile_name="server"`) :
| Condition | Niveau | Déduction |
|-----------|--------|-----------|
| Pas de swap configuré | INFO | aucune |
| Swap sur SSD + swappiness > 30 | WARN | −1 pt |
| Swap ≥ 32 Mo ET RAM libre > 50% ET swappiness > recommandé | WARN | aucune |
| swappiness sous-optimal (défaut 60) | INFO | aucune |
| swappiness optimal | OK | aucune |

Swappiness recommandée selon le profil : server → 1 ; workstation → 10.
Le swap injustifié requiert les 3 conditions pour éviter les faux positifs dus au vieillissement LRU par kswapd.

**Domaine** : les résultats avec le préfixe `memory` sont rattachés à `hardening` via `_PREFIX_TO_DOMAIN`.

**Constantes** : `_SSD_SWAPPINESS_THRESHOLD = 30`, `_RAM_FREE_THRESHOLD = 0.50`, `_MIN_SWAP_USED_KB = 32 * 1024`

### Locale (`en.json`, `fr.json`)

**Clés disk** (27) : `disk.ok`, `disk.smartctl_missing`, `disk.smartctl_missing_detail`, `disk.smart_ok`, `disk.smart_failed`, `disk.smart_failed_detail`, `disk.smart_failed_reason`, `disk.smart_virtual`, `disk.smart_unknown`, `disk.reallocated_sectors`, `disk.reallocated_sectors_detail`, `disk.reallocated_sectors_reason`, `disk.pending_sectors`, `disk.pending_sectors_detail`, `disk.pending_sectors_reason`, `disk.uncorrectable_errors`, `disk.uncorrectable_errors_detail`, `disk.uncorrectable_errors_reason`, `disk.partition_critical`, `disk.partition_critical_detail`, `disk.partition_critical_reason`, `disk.partition_warn`, `disk.smart_tips`, `disk.smart_tips_detail`, `disk.col_mountpoint`, `disk.col_size`, `disk.col_used`

**Clés memory** (9) : `memory.no_swap`, `memory.swap_stats`, `memory.swappiness_ok`, `memory.swappiness_ssd_wear`, `memory.swappiness_ssd_detail`, `memory.swappiness_unjustified`, `memory.swappiness_unjustified_detail`, `memory.swappiness_suboptimal`

**Clés sections** : `sections.memory = "MEMORY & SWAP"`, `sections.disk = "DISK HEALTH"`

### Affichage (`display.py`)

- `display_disk_partitions(snapshot, t, output_module)` — tableau partitions dans la section ÉTAT DES DISQUES
  - Colonnes : point de montage, appareil, taille (Go), barre de progression colorée, usage%
  - Barre : `█` (rempli) + `░` (vide), 10 caractères ; couleur : vert < 70%, jaune < 90%, rouge ≥ 90%
  - Taille : `< 1 Go` si < 0,5 Go, sinon `N Go` ; helpers `_disk_bar()` et `_gb_str()`
  - Les lignes de données utilisent `print()` directement (pas `print_dim`) pour préserver les codes ANSI

### Runner (`runner.py`)

- `display_disk_partitions` importé et appelé après `display_result(disk_result, ...)` dans `if not config.quiet:`
- `--explain` list : affichage groupé avec 15 sections via `_EXPLAIN_GROUPS`

### `--explain` (`explain.py`)

- `_EXPLAIN_GROUPS` : 15 groupes, 63 clés — source unique dont `EXPLAIN_KEYS` est dérivé
- Nouveaux groupes : SSH — Clés autorisées (5 clés), SSH — Config client (3 clés), Règles pare-feu (2 clés), IPv6 (2 clés), Politique mots de passe (2 clés), Disque (3 clés), Mémoire (2 clés)
- `run_explain(key="list")` itère sur `_EXPLAIN_GROUPS` et affiche les en-têtes de groupes
- `tests/test_explain.py` : `test_has_sixty_three_keys` vérifie `len(EXPLAIN_KEYS) == 63`

### Tests

- `tests/test_disk.py` — 60 tests sur 9 classes (+3 tests NVMe) : `TestSnapshotDefaults`, `TestSmartctlMissing`, `TestSmartVirtual`, `TestSmartUnknown`, `TestSmartPassed`, `TestSmartFailed`, `TestSmartAttributes`, `TestNvmeAttrs`, `TestMultipleDisks`, `TestPartitionUsage`, `TestAllClear`, `TestParseSmartAttr`, `TestEdgeCases`
- `tests/test_memory.py` — 37 tests sur 8 classes (+6 tests robustesse) : `TestSnapshotDefaults`, `TestNoSwap`, `TestSsdWear`, `TestUnjustifiedSwap`, `TestSuboptimalSwappiness`, `TestProfileAware`, `TestSwapStats`, `TestEdgeCases`
...
- Passage qualité sur domain_scores, updates, webhook, explain
...
### Résumé
- Bannière enrichie : version du noyau, version iptables + backend, version nftables
- Nouvelle section **ANALYSE DE LA PILE PARE-FEU** : contournement iptables brut, règles nftables parallèles, ip_forward inattendu
- Nouvelle section **CONTEXTE RÉSEAU** : tableau d'interfaces (type/état/IP) + connexions TCP établies
- Sortie JSON (`--json-full`) étendue avec les objets `firewall_stack` et `network_context`
- Passage qualité : 12 modules renforcés (correction XSS, écriture atomique, validation des entrées, gardes CLI, fallback i18n…)
- 766/766 tests unitaires (+89)

### Nouvelles sections

- **ANALYSE DE LA PILE PARE-FEU** (`checks/firewall_stack.py`) — nouvelle vérification après l'analyse des règles UFW :
  - Règles `ACCEPT` brutes dans la chaîne INPUT iptables ne sautant pas vers une chaîne `ufw-*` → WARN + 2 points de déduction par règle
  - Règles `ACCEPT` brutes dans FORWARD : WARN si aucun Docker/WireGuard/libvirt détecté ; INFO si démon de routage présent
  - Tables nftables en parallèle d'UFW : signalées uniquement si des tables non-UFW, non-compat-iptables existent (`filter`, `nat`, `mangle`, `raw`, `security` sont des traductions iptables-nft — exclues)
  - `ip_forward=1` sans Docker, WireGuard ni libvirt/KVM → WARN + 1 point de déduction
  - libvirt/KVM détecté via binaire `libvirtd`/`virtqemud` ou répertoire `/var/lib/libvirt`

- **CONTEXTE RÉSEAU** (`checks/network_context.py`) — nouvelle vérification après Analyse de la pile pare-feu :
  - Tableau d'interfaces : toutes les interfaces non-loopback, non-veth avec nom, type catégorisé (ethernet/wifi/bridge/tunnel/other), état opérationnel (ACTIF/INACTIF depuis le champ `state`), adresse IPv4 principale
  - Connexions TCP établies : nombre + top 3 IP externes (depuis `ss -tnp state established`)
  - Interface tunnel active (tun/tap ACTIF) → INFO
  - Connexion établie vers IP externe sur port sensible (MySQL 3306, PostgreSQL 5432, Redis 6379, MongoDB 27017, CouchDB 5984) → WARN + 2 points de déduction

### Bannière enrichie

- `sysinfo.py` — `get_system_info()` collecte désormais :
  - `iptables_version` : depuis `iptables --version`, capture la version + backend ex. `"1.8.10 (nf_tables)"`
  - `nftables_version` : depuis `nft --version`, capture la version ex. `"1.0.9"`
  - Chaîne vide retournée si l'outil n'est pas installé
- `report.py` — dataclass `SystemInfo` étendue avec `iptables_version: str` et `nftables_version: str`
- `output.py` — `print_banner()` étendu : colonne labels élargie à 14 caractères ; lignes `kernel`, `iptables`, `nftables` ajoutées ; `"non installé"` affiché si vide
- `locales/en.json` + `fr.json` — nouvelles clés : `banner.kernel`, `banner.iptables`, `banner.nftables`, `banner.not_installed`, `sections.firewall_stack`, `sections.network_context`, `firewall_stack.*` (7 clés), `network_context.*` (14 clés)

### Sortie JSON

- `json_output.py` — `build_json_data()` étendu (mode full) :
  - `"firewall_stack"` : `input_bypasses` (liste), `forward_bypasses` (liste), `nftables_active` (bool), `ip_forward` (bool), `docker_present`, `wireguard_present`, `libvirt_present`
  - `"network_context"` : `interfaces` (liste de `{name, type, up, address}`), `connections_count` (int), `top_remote_ips` (liste de `[ip, count]`)

### Tests

- `tests/test_firewall_stack.py` — 38 tests (nouveau fichier) : `TestCleanSystem`, `TestInputChainBypass`, `TestForwardChain`, `TestNftables`, `TestIPForwarding`, `TestParseRawAccepts`, `TestHasUserNftRules`
- `tests/test_network_context.py` — 51 tests (nouveau fichier) : `TestCheckNetworkContext`, `TestTunnelInterface`, `TestSensitiveRemotePort`, `TestInterfaceType`, `TestParseInterfaces`, `TestParseConnections`, `TestSplitAddrPort`, `TestIsPrivateOrLoopback`, `TestTopRemoteIps`
- `tests/test_report.py` — fixture `SystemInfo` mise à jour avec `iptables_version="1.8.9"`, `nftables_version=""`

### Passage qualité

Douze modules renforcés sans changement de comportement sur un audit propre. Chaque correction répond à une cause de défaillance ou une préoccupation de sécurité identifiée :

| Module | Corrections |
|--------|-------------|
| `report_markdown.py` | Correction XSS via URL de lien (`_safe_url()` n'autorise que `http://`/`https://`) ; cohérence des horodatages (`created_at` défini une fois dans `open()`, réutilisé dans l'en-tête) ; normalisation des colonnes de tableau ; extension `.log` → `.md` |
| `registry.py` | Plage de ports 1–65535 appliquée (rejette `99999/tcp`) ; garde contre les mots-clés Python dans `config_key` (`keyword.iskeyword()`) ; `__iter__` typé `-> Iterator[Service]` ; ordre plugins-avant-init documenté |
| `cli.py` | `--lang=CODE` accepte n'importe quel code (`--french` conservé comme alias) ; `--quiet`+`--json` lève `CLIError` ; `--json`+`--fix` lève `CLIError` ; `--log-days` plafonné à 3650 ; import `field` inutilisé supprimé ; placement de la docstring `no_color` corrigé |
| `config.py` | Écriture atomique via `.tmp` + `Path.replace()` pour `UserConfig._save()` et `EmailStore._save()` (prévient la corruption en cas de crash) ; email validé avec regex stricte ; `set()` rejette les clés non-identifiants |
| `cron.py` | Regex email renforcé (même motif que `config.py`) ; parseur de ligne cron capture le chemin du script avec `(.+)$` + `.strip()` (supporte les espaces) ; `_validate_custom_cron()` vérifie minute (0–59) et heure (0–23) |
| `fixes.py` | La liste `manual_items` est désormais affichée après les auto-fixes (était collectée mais jamais montrée — bug UX) ; regex de suppression UFW ancrée : `^(?:sudo\s+)?ufw\s+.*--force\s+delete\s+\d+$` ; garde contre les opérateurs shell ; `cmd.replace("\n"," ")` avant affichage ; clé `done_summary` ajoutée (EN + FR) |
| `i18n.py` | Fallback par clé vers l'anglais : si la locale FR est chargée, les clés manquantes tombent sur l'anglais plutôt que `[clé]` ; `_load_locale()` valide que la racine JSON est un `dict` ; garde de profondeur de clé (`_MAX_KEY_DEPTH = 10`) ; `_load_locale()` extrait pour éviter la duplication ; `logger.debug` utilise `_lang` (langue réellement chargée) |
| `manage_logs.py` | La suppression `"all"` demande une confirmation explicite `[y/N]` ; clé `manage_logs.confirm_all` ajoutée (EN + FR) |
| `panorama.py` | `PanoramaRow TypedDict` remplace `dict` opaque ; garde `getattr(snap, "state", None)` ; garde `snap.exposures or {}` ; `str(p)` pour les valeurs de ports ; `risk` normalisé en minuscules |
| `completion.py` | `src.exists()` vérifié avant `shutil.copy2` (erreur claire si le fichier de données est absent après installation) ; messages distincts selon la cause d'échec du symlink |
| `_paths.py` | `.strip()` sur la variable d'environnement `UFW_AUDIT_SHARE` ; `resolve(strict=True)` lève une erreur tôt sur les chemins inexistants |
| `pyproject.toml` | `readme = { file = "README.md", content-type = "text/markdown" }` (type MIME explicite pour PyPI) ; classifier Python 3.11 ajouté |
...
- **NetBIOS 137/138 signalé malgré une règle UFW** (`checks/ports.py`) — Dans `_categorize_port()`, la branche `NETBIOS` était évaluée avant la vérification de couverture UFW. Tout port dans `(137, 138)` retournait toujours `PortCategory.NETBIOS` quel que soit l'existence d'une règle UFW. Déplacement de la vérification `_is_covered_by_ufw()` avant la branche NetBIOS, de sorte qu'un port couvert retourne `COVERED` sans déclencher d'avertissement ni de déduction.
