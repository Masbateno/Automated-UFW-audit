...
from ufw_audit.scoring import FindingLevel, ScoreEngine
...
def build_html_output(engine: ScoreEngine, sys_info: SystemInfo) -> str:
    ...
    a(f"<title>ufw-audit — {_h(sys_info.hostname)} — {_h(ts)}</title>")
    a(f"<style>{_CSS}</style>")
    a("</head>")
    a("<body>")

    a(f"<h1>ufw-audit Report — {_h(sys_info.hostname)}</h1>")
    ...
    a('<footer>Generated by <a href="https://github.com/Masbateno/ufw-audit">ufw-audit</a>.'
