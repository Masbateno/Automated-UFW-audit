"""
Webhook notification module for ufw-audit.
...
Usage:
    from ufw_audit.webhook import send_webhook
...
if TYPE_CHECKING:
    ...
    from ufw_audit.sysinfo import SystemInfo
...
def build_slack_payload(engine: "ScoreEngine", sys_info: "SystemInfo",
    ...
    summary = (
        f"*ufw-audit* v{version} — `{sys_info.hostname}`\n"
    ...
    return {
        "text": f"ufw-audit report — {sys_info.hostname}",
        "attachments": [
            {
                "color":  color,
                "text":   summary,
