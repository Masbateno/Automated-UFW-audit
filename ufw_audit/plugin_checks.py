"""
Plugin check loader for ufw-audit.

Scans ~/.config/ufw-audit/checks.d/ for Python files that implement a
...
Security note
-------------
Plugins run as root (ufw-audit requires root).  Only place files in
checks.d/ that you trust.  ufw-audit validates the file size and the
presence of ``run_check`` before loading, but does not sandbox execution.

Example plugin (~/.config/ufw-audit/checks.d/motd_check.py)
-------------------------------------------------------------
...
_PLUGIN_CHECKS_DIR = Path.home() / ".config" / "ufw-audit" / "checks.d"
...
def load_plugin_checks(
    ...
    Args:
        plugin_dir: Override the default ~/.config/ufw-audit/checks.d/.
                    Useful in tests.
...
def _load_one(plugin_path: Path) -> PluginCheck | None:
    ...
    module_name = f"ufw_audit_plugin_{plugin_path.stem}_{abs(hash(plugin_path))}"
    try:
