...
def _run_install_cron_plain(user_config, config, t) -> int:
    ...
    script_content = (
        ...
        "    except OSError:\n"
        "        pass\n"
        "subject = f'[UFW-AUDIT] {hostname} - Score {score}'\n"
    ...
    while True:
        ...
        while True:
            if step == LANDING:
                ...
                h, w = stdscr.getmaxyx()
                try:
                    stdscr.addstr(0, 0, "  ufw-audit --install-cron".ljust(w - 1), hdr_attr)
        ...
        now_str   = datetime.now().strftime("%Y-%m-%d")
        try:
            ufw_audit_path = str(_Path(__file__).parent.parent)
        except (TypeError, AttributeError):
            ufw_audit_path = "/usr/local/lib"
        ...
        root_config_path = _Path("/root/.config/ufw-audit/config.conf")
        try:
            from ufw_audit.config import UserConfig as _UC
            root_cfg = _UC.load(path=root_config_path)
            if not root_cfg.get("log_dir"):
...
def _run_manage_cron_curses(stdscr, config, t) -> int:
    ...
    while True:
        ...
        ftr_attr = _c.color_pair(2) if has_color else _c.A_REVERSE
        if confirm_delete:
            header = "  ufw-audit --manage-cron    Confirm deletion below  "
        elif n_sel:
            header = (f"  ufw-audit --manage-cron    "
                      f"↑↓: move   Spc: toggle   a: all   u: unmark   "
                      f"d: delete ({n_sel})   q: quit")
        else:
            header = ("  ufw-audit --manage-cron    "
